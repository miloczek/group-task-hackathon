...
def closest_departures(city):
    ...
    if not start_coords or not end_coords:
        abort(400, description="Both start_coordinates and end_coordinates are required")
        
    # Validate coordinate format (latitude,longitude)
    coord_pattern = re.compile(r'^-?\d+\.?\d*,-?\d+\.?\d*$')
    if not coord_pattern.match(start_coords) or not coord_pattern.match(end_coords):
        abort(400, description="Coordinates must be in format: latitude,longitude")
    ...
    start_time = request.args.get('start_time', datetime.now().strftime("%H:%M"))
